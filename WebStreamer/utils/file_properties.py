...
async def gen_link(m: Message,log_msg: Messages, from_channel: bool):
    ...
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶ Assistir", url=page_link), InlineKeyboardButton("📥 Fazer Download", url=stream_link)]])

    if from_channel:
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶ Assistir", url=page_link), InlineKeyboardButton("📥 Fazer Download", url=stream_link)]])
    else:
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("▶ Assistir", url=page_link), InlineKeyboardButton("📥 Fazer Download", url=stream_link)],
            [InlineKeyboardButton("❌ Deletar Link", callback_data=f"msgdelconf2_{log_msg.id}_{get_media_file_unique_id(log_msg)}")]])
